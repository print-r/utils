...
let Express = /** @class */ (() => {
    class Express {
        ...
        handleReset() {
            ...
            this.type = 'lately';
            // 获取所有分类
            let childs = document.querySelectorAll('.ex_cate_container li');
            // 排他
        ...
        handleSavelatelyData() {
            ...
            if (localStorage.getItem('expressData')) {
                ...
                if (JSON.stringify(Express.currentData) != '{}')
                    localStorage.setItem('expressData', JSON.stringify([Express.currentData]));
            }
        }
        // 初始化
        init() {
        ...
        handleInitData() {
            // 获取最近使用数据
            if (localStorage.getItem('expressData'))
                this.expressData['lately'] = JSON.parse(localStorage.getItem('expressData'));
            // 基本数据
            expressData.forEach((val, index) => {
                // 添加下标
                val.index = index;
                for (let name in this.expressData) {
                    if (name == 'lately' || name == 'hot')
                        continue;
                    // 筛选数据
        ...
        handleGetData() {
            ...
            if (this.expressData[this.type].length) {
                // 隐藏tips
                $('.ex_tips').eq(0).hide();
                this.expressData[this.type].forEach((val) => {
