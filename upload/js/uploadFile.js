...
class UploadFile {
    ...
    init() {
        ...
        for (let item in this.param) {
            if (_this.specialParams.filter((val) => { return val == item; }).length) {
                //保存拖拽元素参数
        ...
        if (UploadFile.config.drag.el) {
            let tagName = $(UploadFile.config.drag.el)[0].tagName;
        ...
        //绑定change事件
        $(document).unbind("change").on('change', UploadFile.config.el, function (e) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (UploadFile.files.length) {
                if (!UploadFile.config.url)
                    return (layer.msg('文件上传路径有误，请检查是否未填写路径'));
                if (this.runStatus)
                    return (layer.msg('请勿重复提交，正在上传中...'));
                //loading层
                uploadLoading = layer.load(0, {
                    shade: [0.5, '#fff'],
                });
                typeof callBack == 'function' ? callBack(yield this._handleSendUpload()) : yield this._handleSendUpload();
            }
            else {
                layer.msg('请先选择图片');
            }
    ...
    _handleSendUpload() {
        ...
        if (UploadFile.config.isDirectUpload) {
            //直接上传
            data.append(UploadFile.config.param, UploadFile.files[UploadFile.files.length - 1]);
        }
        else {
            //file二进制文件参数（提交上传）
            UploadFile.files.forEach((val, key) => {
    ...
    _handle() {
        let _this = this;
        //图片预览、删除图片
        $(UploadFile.config.drag.el || this.imgShowEl).unbind("click").on('click', '.drag_del,.drag_pre', function () {
    ...
    _handleDragUpload() {
        ...
        if (drag.el) {
            ...
            //兼容谷歌火狐
            $(document).unbind("dragover").bind('dragover', drag.el, function (e) {
     ...
     * @param files 文件数组对象
     */
    _handleSaveFiles(files) {
        //图片上传最大数
        let maxFile = UploadFile.config.maxFile;
    ...
    _handleAsync(file) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let res = yield this._handleCheckFile(file);
            //保存文件
            if (res !== false) {
                //保存文件数据
                UploadFile.files.push(file);
                //当前上传的下标
                let index = UploadFile.files.length - 1;
                //创建元素
                let el = document.createElement(UploadFile.element);
                //设置元素下标
    ...
    _handleCheckFile(file) {
        ...
        //验证图片类型
        let check = UploadFile.config.suffix.length ? UploadFile.config.suffix.filter((val) => file.name.toLowerCase().indexOf(val) != '-1') : [true];
        if (check.length <= 0) {
        ...
        return new Promise((resolve, reject) => {
            ...
            //判断图片是否重复
            return new Promise((resolve, reject) => {
                UploadFile.baseFiles.forEach((val) => {
                    if (!isRepeat) {
                        if (val.src == baseFile)
                            isRepeat = true;
                    }
                ...
                if (UploadFile.config.specs.length) {
                    return new Promise((resolve, reject) => {
                        ...
                        img.onload = function () {
                            //图片尺寸验证
                            UploadFile.config.specs.forEach((val) => {
                                var size = val.split('*');
                                //验证图片尺寸
